...
インストールもログインも不要な、ブラウザ完結の文字起こしツールです。音声ファイル、動画の音声トラック、マイク録音を端末内の Whisper で処理し、入力データと文字起こし結果はアプリのサーバーへ送信しません。
...
- 90秒以内・25 MB以内の音声ファイルを文字起こし
- 90秒以内・100 MB以内の動画から音声トラックを端末内で取り出して文字起こし
- ブラウザから最大90秒のマイク録音
- 日本語、英語、中国語を明示選択
...
音声・動画・マイク録音のアップロード、ユーザー登録、解析タグ、クラウド保存はありません。

## データの流れ

```text
音声ファイル / 動画の音声トラック / マイク録音
  → ブラウザ内でデコードし、AudioContext で 16 kHz / mono に変換
  → Web Worker 内の Whisper (ONNX Runtime Web) で推論
  → 文字起こし結果を画面に返す
```

アプリは入力データと結果を永続保存せず、サーバーへも送信しません。処理中はブラウザがメモリや内部の一時領域に素材を保持することがあります。初回のみ Hugging Face からモデルを取得し、モデルファイルはブラウザの Cache API に保存されることがあります。モデル取得は `huggingface.co` から Hugging Face の CDN（現在は `*.cdn.hf.co` 等）へリダイレクトされる場合があります。社内プロキシではリダイレクト先を含む許可が必要です。

サイトのホスティング/CDN事業者は、ページやアセットの配信時にIPアドレス、User-Agent、リクエストURL、時刻など通常のアクセスメタデータを処理・記録する場合があります。音声・動画・マイク録音と文字起こし結果はその通信に含まれません。アプリの解析タグと Workers Observability は無効です。
...
- Firefox / Safari: WASM / CPU で動作する設計ですが、処理速度、録音機能、コーデックの対応は環境依存です
- 音声: WAV、MP3を正式対応。M4A、AAC、WebM、OGGはブラウザがコーデックをデコードできる場合に利用可能
- 動画: 現行ChromeでMP4（H.264 / AAC）とWebM（VP9 / Opus）を実測済み。Edgeを含め、MOVなどその他のコンテナやコーデックはブラウザの対応状況に依存
- マイク録音: HTTPS上で、利用者が録音ボタンを押した後にブラウザのマイク許可が必要。要求するのは音声入力だけで、カメラや画面共有は要求しません

会社PCではマイク、WebGPU、Web Worker、WebAssembly、`huggingface.co` への通信、ブラウザキャッシュが組織ポリシーで制限される場合があります。「インストール不要」と「社内規定上利用可能」は別なので、録音対象者の同意を得たうえで、利用前に所属組織のルールを確認してください。
...
- 話者分離、ライブ文字起こし、翻訳は未対応です。マイク録音は停止後にまとめて文字起こしします
- システム音声やブラウザタブの音声を直接録音する機能はありません
- 動画は音声トラックだけを文字起こしします。対応するコンテナやコーデックはブラウザに依存し、音声ファイルとしての書き出しは行いません
- 字幕用タイムスタンプは出しません
